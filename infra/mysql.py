...
async def new_pool(loop):
    pool = await aiomysql.create_pool(
        host=config.MYSQL.HOST, 
        port=config.MYSQL.PORT,
        user=config.MYSQL.USER,
        password=config.MYSQL.PASSWORD,
        db=config.MYSQL.DB, loop=loop)
    return pool
...
async def init_pool(loop):
    pool = await new_pool(loop)
    set_pool(pool)
    logger.info("<infra> mysql initialized")
...
'''
--------- IMPOARTANT ---------
 DON'T SET autocommit AS True
 OTHERWISE INSERT DOESN'T WORK
--------- IMPOARTANT ---------
'''
async def execute(sql, args=None, autocommit=False):
    log(sql)
    async with __pool.get() as conn:
        if not autocommit:
            await conn.begin()
...
class ModelMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        logger.info('<mysql> found model: %s (table: %s)' % (name, tableName))
        mappings = dict()
        fields = []
        primaryKey = None
        for k, v in attrs.items():
            if isinstance(v, Field):
                logger.info('<mysql>  found mapping: %s ==> %s' % (k, v))
                mappings[k] = v
                if v.primary_key:
                    # 找到主键:
                    if primaryKey:
        ...
        for k, v in attrs.items():
            if isinstance(v, Field):
                createStmt += f"{k} {v.column_type} NOT NULL,"
        createStmt += f"PRIMARY KEY ({primaryKey})"
        createStmt += ")"
        ...
        create_stmts.append(createStmt)
        attrs['__create_stmt__'] = createStmt
